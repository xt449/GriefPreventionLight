...
import com.github.xt449.griefpreventionlight.events.AccrueClaimBlocksEvent;
...
class DeliverClaimBlocksTask implements Runnable {
	...
	public DeliverClaimBlocksTask(Player player, GriefPreventionLight instance) {
		this.player = player;
		...
		try {
			...
			if(isIdle) {
				if(instance.config_claims_accruedIdlePercent <= 0) {
					GriefPreventionLight.AddLogEntry(player.getName() + " wasn't active enough to accrue claim blocks this round.", CustomLogEntryTypes.Debug, true);
			...
			if(event.isCancelled()) {
				GriefPreventionLight.AddLogEntry(player.getName() + " claim block delivery was canceled by another plugin.", CustomLogEntryTypes.Debug, true);
			...
			GriefPreventionLight.AddLogEntry("Delivering " + event.getBlocksToAccrue() + " blocks to " + player.getName(), CustomLogEntryTypes.Debug, true);
			...
			GriefPreventionLight.AddLogEntry("Problem delivering claim blocks to player " + player.getName() + ":");
			e.printStackTrace();
		}
	}
}
