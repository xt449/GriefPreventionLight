...
import com.github.xt449.griefpreventionlight.events.ClaimExpirationEvent;
...
class CleanupUnusedClaimTask implements Runnable {
	...
	public void run() {
		...
		if(GriefPreventionLight.instance.config_claims_automaticClaimsForNewPlayersRadius >= 0) {
			areaOfDefaultClaim = (int) Math.pow(GriefPreventionLight.instance.config_claims_automaticClaimsForNewPlayersRadius * 2 + 1, 2);
		}

		//if this claim is a chest claim and those are set to expire
		if(claim.getArea() <= areaOfDefaultClaim && GriefPreventionLight.instance.config_claims_chestClaimExpirationDays > 0) {
		...
		else if(GriefPreventionLight.instance.config_claims_expirationDays > 0) {
