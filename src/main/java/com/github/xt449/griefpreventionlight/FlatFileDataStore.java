...
public class FlatFileDataStore extends DataStore {
				...
				try {
					...
					while(line != null) {
						...
						if(topLevelClaim == null) {
							//instantiate
							topLevelClaim = new Claim(lesserBoundaryCorner.getWorld(), Coordinate.fromLocation(lesserBoundaryCorner), Coordinate.fromLocation(greaterBoundaryCorner), ownerID, builderNames, containerNames, accessorNames, managerNames, claimID);
						...
						//otherwise there's already a top level claim, so this must be a subdivision of that top level claim
							...
							Claim subdivision = new Claim(lesserBoundaryCorner.getWorld(), Coordinate.fromLocation(lesserBoundaryCorner), Coordinate.fromLocation(greaterBoundaryCorner), null, builderNames, containerNames, accessorNames, managerNames, null);
	...
	Claim loadClaim(String input, ArrayList<Long> out_parentID, long lastModifiedDate, long claimID, List<World> validWorlds) throws Exception {
		...
		//instantiate
		claim = new Claim(lesserBoundaryCorner.getWorld(), Coordinate.fromLocation(lesserBoundaryCorner), Coordinate.fromLocation(greaterBoundaryCorner), ownerID, builders, containers, accessors, managers, inheritNothing, claimID);
